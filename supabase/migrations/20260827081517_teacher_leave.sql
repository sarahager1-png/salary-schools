-- יציאה לחופשה, עם תאריך.
--
-- חל"ד היה קיים במערכת רק מהצד השני: REASON_TYPES של מילוי מקום ידע
-- לומר *למה* מישהי נכנסה, אבל לא היה איפה לרשום שמורה קיימת יוצאת,
-- וממתי. זה נתון שהמנהלת יודעת ראשונה, והוא משנה את השכר.
--
-- leave_to ריק = פתוח, טרם נקבע תאריך חזרה.
alter table public.teacher_months
  add column leave_type text not null default 'none'
    check (leave_type in ('none', 'maternity', 'unpaid', 'sick', 'other')),
  add column leave_from date,
  add column leave_to   date;

comment on column public.teacher_months.leave_type is
  'none | maternity (חל"ד) | unpaid (חל"ת) | sick (מחלה ממושכת) | other';

-- חופשה חייבת תאריך התחלה, ותאריך חזרה אינו לפני היציאה
alter table public.teacher_months
  add constraint leave_needs_from check (leave_type = 'none' or leave_from is not null),
  add constraint leave_dates_order check (leave_to is null or leave_from is null or leave_to >= leave_from);

-- ── המנהלת רושמת את זה, כמו כל שדה בסיס אחר ─────────────────
-- הטריגר אוסר על מנהלת רק את עמודות הכסף והאישורים; שדות החופשה אינם
-- שם, ולכן היא רשאית. דרך הקישור צריך להוסיף אותם במפורש לשתי
-- הפונקציות, שמעדכנות רשימת עמודות סגורה.
...
begin
  ...
  -- יציאה לחופשה משנה את השכר, ולכן נחשבת שינוי בסיס
  base_changed :=
  ...
  update public.teacher_months set
    ...
    -- מחרוזת ריקה מנקה את התאריך; היעדר המפתח משאיר אותו
