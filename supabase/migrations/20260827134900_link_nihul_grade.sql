-- דרגת ניהול עוברת גם דרך הקישור.
--
-- המנהלת מזינה את עצמה, ודרגת הניהול היא שדה שלה בלבד. בלי המעבר הזה
-- היא הייתה בוחרת דרגה במסך ושום דבר לא היה נשמר — שדה שאינו ברשימת
-- ה-RPC פשוט נופל בשקט.
--
-- שינוי בדרגת ניהול הוא שינוי בסיס: הוא משנה שכר, ולכן מאפס סימולציה
-- ואישור, כמו ותק או דרגה.
