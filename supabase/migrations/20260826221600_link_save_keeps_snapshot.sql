-- שינוי דרך הקישור לא הותיר עקבות.
--
-- במסלול המחובר, שינוי בשדה בסיס שומר צילום "לפני" של השורה, והשליח
-- רואה במסך האישור "ותק: 8 → 11". דרך הקישור הצילום לא נשמר כלל: שתי
-- שורות זו לצד זו, אחת עם פירוט שינויים ואחת בלי כלום — והשנייה היא
-- דווקא זו שהמנהלת שינתה. השליח התבקש לאשר שכר בלי לדעת מה זז.
--
-- הצילום נלקח בלקוח (הוא בשמות של הממשק, לא של המסד) ונשמר כאן פעם
-- אחת בלבד, כמו במסלול המחובר: הצילום הראשון הוא ה"לפני" האמיתי, וכל
-- שינוי נוסף לפני האישור נמדד מולו ולא מול עצמו.
...
begin
  ...
  update public.teacher_months set
    ...
    -- הצילום נשמר פעם אחת: הראשון הוא ה"לפני" האמיתי מול האישור
