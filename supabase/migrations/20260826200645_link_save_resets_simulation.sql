-- שמירה דרך הקישור לא איפסה את הסימולציה.
--
-- מנהלת ששינתה שעות דרך הקישור השאירה על השורה את השכר שחושב לשעות
-- הישנות. השורה נראתה כאילו הסימולציה הושלמה, לא חזרה לתור של חשבת
-- השכר, והמשיכה לאישור עם מספר שאינו תואם את הנתונים שלצידו.
--
-- במסלול המחובר הכלל הזה כבר קיים — שינוי שדה בסיס מבטל סימולציה
-- ואישורים. כאן הוא חסר, ולכן אותה פעולה בדיוק נתנה שתי תוצאות
-- שונות לפי איך שנכנסת. בנוסף, כל שמירה ביטלה אישור גם כשלא השתנה
-- דבר בשדות הבסיס — עדכון ימי היעדרות לבדו החזיר שורה מאושרת לתור.
...
begin
  ...
  base_changed :=
    ...
    or coalesce((p_row ->> 'grade')::int,             target.grade)             is distinct from target.grade
  ...
  update public.teacher_months set
    ...
    grade             = coalesce((p_row ->> 'grade')::int, grade),
