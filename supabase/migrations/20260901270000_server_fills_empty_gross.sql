-- השרת רשאי למלא ברוטו ריק — ולעולם לא לדרוס קיים.
--
-- "אתה לא עושה סימולציות באוטומט?" (שרה, 1.9). המרַיץ נוהג במחשבון של
-- משרד החינוך וממלא ברוטו לשורות שעדיין אין להן. הוא רץ עם מפתח השרת,
-- והטריגר חסם בצדק כל כתיבת שכר מהשרת. הפתח שנפתח כאן צר בכוונה:
-- official_gross נכתב מהשרת רק כשהערך הקודם ריק. מספר שהוזן ביד —
-- על ידי חשבת השכר או שרה — אינו ניתן לדריסה אוטומטית לעולם.
--
-- שאר הפונקציה זהה למקור שב-20260901230000_clerk_may_fix_facts.
