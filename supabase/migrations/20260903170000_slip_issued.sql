-- "אסתר תוכל לסמן למי כבר הוציאה תלוש וי" (שרה, 3.9.2026).
--
-- payroll_ready תפוס — הוא "עבר לשכר" של מחזור הדיווח (הקרון כותב אותו).
-- הוצאת תלוש היא עובדה אחרת, של חשבת השכר, ולכן עמודה משלה: התאריך הוא
-- גם הווי וגם מתי סומן. null = טרם הוצא.
alter table public.teacher_months
  add column if not exists slip_issued_at timestamptz;

comment on column public.teacher_months.slip_issued_at is
  'וי של חשבת השכר — התלוש לחודש הזה כבר הוצא';

-- "אסור שאסתר תדרוס נתונים — שתכתוב כמה שיצא לה בעמודה ליד" (שרה, 3.9):
-- הברוטו שיצא בתלוש בפועל נרשם בעמודה משלו, לצד המספר של המערכת,
-- ואינו נוגע בו.
alter table public.teacher_months
  add column if not exists slip_gross numeric check (slip_gross is null or slip_gross > 0);

comment on column public.teacher_months.slip_gross is
  'הברוטו שיצא בתלוש בפועל — חשבת השכר רושמת, לצד המספר של המערכת';

-- חשבת השכר והשליח מסמנים; מנהלת בית ספר לא.
