/*
  ...
  הכתיבה במפתח השרת, וההגנה במסד: הטריגר מרשה לשרת לכתוב official_gross
  רק כשהערך הקודם ריק (מיגרציה 20260901270000). המרַיץ ממלא — לעולם
  אינו דורס מספר שהוזן ביד.
*/
...
// ── ריצה ─────────────────────────────────────────────────────
// מפתח השרת. ההגנה מפני דריסה יושבת בטריגר במסד, לא בנימוס של הסקריפט.
...
try {
  ...
  for (const { t, plan } of todo) {
    ...
    console.log(`✓ ${t.name.padEnd(20)} ${t.reform === 'ofek' ? 'אופק' : 'עולם ישן'} · דרגה ${plan.darga} · ותק ${plan.vetek} · ${plan.pct}%${plan.kita ? ' · מחנכת' : ''} → ${gross.toLocaleString('he-IL')} ₪ · ${saved}`);
  }
