/*
  בדיקת הברוטו מול המחשבון הרשמי.

  המערכת אינה מחשבת שכר — חשבת השכר מזינה אותו. הכלי הזה בודק את מה
  שהוזן: הוא לוקח כל שורה שיש לה ברוטו, שולח את הנתונים שלה לטופס של
  משרד החינוך בדפדפן אמיתי, ומשווה. פער — נאמר, עם הנתונים ששלחנו.

  זו הדרך לענות על "איך אני בודקת את הסימולציות שעשיתי": מריצים, ומה
  שתואם — תואם. פער אינו בהכרח טעות בברוטו; הוא יכול להיות ותק, דרגה
  או אחוז משרה שגויים בשורה. הפלט אומר מה נשלח, כדי שאפשר יהיה להכריע.
...
console.log(`חודש ${MONTH} · ${rows.length} שורות · ${todo.length} עם ברוטו להשוואה\n`);
