...
export async function linkWhoami(code) {
  ...
  return { fullName: me.full_name, role: me.role, schoolId: me.school_id, schoolName: me.school_name,
           gender: me.gender, hoursQuota: me.hours_quota ?? null };
}
...
export async function requestTeacherForms(monthKey) {
  ...
  for (const t of approved) {
    ...
    queue.push({
      ...
      // הנוסח המוטמע — "תטמיע באתר במסך שליחת הודעה... הגשה עד ה-8.9" (שרה, 3.9)
      body: `${t.name}, שלום.\n` +
...
export async function loadFinance() {
  const { data, error } = await supabase.from('school_finance')
    .select('school_id, ministry_budget, yieul, teaching_sim, network_support, income_total, expenses_other, detail, note, src, updated_at');
