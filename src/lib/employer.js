...
function calcEmployer(t) {
  ...
  /*
    ...
    **הרצפה היא רשת ביטחון, לא מנוע.** ההכרעה (שרה, 9.9): "מה שהכי
    נכון… הכי קרוב לאסתר". והמפורט אכן קולע — הוא נותן 36.55% בממוצע
    משוקלל, מול 37.5% שמתקבלים מטבלת אסתר כשמתרגמים אותה לברוטו שלנו
    (הברוטו שלה כולל הבראה, ביגוד ומעונות, ולכן גבוה מזה שלנו ב-6.9%;
    28.6% על הברוטו שלה = 37.5% על שלנו). פחות מנקודה אחת הפרש.

    **ובכל זאת 140%** (הכרעת שרה, 9.9 — "תעשה 140 אחוז וזהו"): היא
    נכנסת אצל 41 מ-82 ומביאה את התוצאה ל-48.4%, כלומר כרית מעל המדוד.
    זו הכרעה תקציבית מודעת ולא תוצאה של חישוב — עד שחשבת השכר תזין
    עלות בפועל, שגוברת על הכול ממילא.
    ההיסטוריה: 140% (29.8) → 150% (7.9) → 135% → 142% → 130% → 140%
    (9.9). ה-150% פיצה על ברוטו מתוכנן נמוך, וזה התיקון הלא נכון —
  ...
  */
  ...
  // "אין מ"מ שוטף" (שרה, 22.9): מילוי מקום לפי שעות אינו משולם ברשת —
  // ה-5% מילוי מקום לבית ספר (MM_PCT) מכסים אותו. נשארת רק מחליפת חל"ד,
  // שמקבלת את שכרה הרגיל. השורה נשארת לתיעוד בלבד.
  void coversMaternity;
  const mmPay = 0;
