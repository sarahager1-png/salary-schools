...
function calcEmployer(t) {
  ...
  //
  // 8.9 — מה שנמדד מאז, ולמה 150% נשארת בכל זאת. "דו"ח עלות עבודה 9/25"
  // של בית ספר קטמון (שלהבות ירושלים) נותן עלות מעביד של 34.70% מהברוטו,
  // והוא *אינו* כולל הבראה, ביגוד ומעונות (שרה, 8.9). בתוספתם האמת היא
  // 42.2%, והמפורט כאן מגיע ל-40.74% — כלומר הרכיבים נכונים, והרצפה
  // הנכונה לפי המדידה היא 140% ולא 150%. אבל ב-150% הרצפה גוברת על
  // המפורט אצל *כל* 80 המורות, והפער שמעל 42% הוא בדיוק הפיצוי על ברוטו
  // מתוכנן שנמוך ב-15% מהברוטו שיוצא בתלוש (נמדד שוב 8.9: אותן שלוש
  // מורות ברמת ישי, 520 ₪ ברוטו לשעה שנה שעברה מול 452 היום — יחס 1.151).
  // שני העיוותים מתקזזים ומביאים את אופק ל-671 ₪ לשעה מול 682 שנמדדו
  // בפועל. לכן ההכרעה (שרה, 8.9): **להשאיר 150% עד שחשבת השכר תייבא
  // ברוטו אמיתי** דרך מסך ייבוא התלוש. מאותו רגע הברוטו נכון, הרצפה
  // הופכת לניפוח כפול — ואז מורידים ל-140%.
