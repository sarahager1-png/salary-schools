...
/*
  ...
  חני מור יוסף, רעננה — "היום עטייה" (שרה, 22.9). כל הכתיבים של השם מתכנסים
  לשם אחד, בשני הצדדים (words חל גם על שם העובדת), כדי שתלוש על כל אחד מהם
  יתאים — גם לפני וגם אחרי שינוי השם במערכת.
*/
const ALIASES = {
  'אורנשטיין יעל': 'ליפשיץ יעל',
  'מור יוסף חני': 'עטייה חני', 'חני מור יוסף': 'עטייה חני',
  'מור יוסף חנה': 'עטייה חני', 'חנה מור יוסף': 'עטייה חני',
  'אטיאס חני': 'עטייה חני', 'חני אטיאס': 'עטייה חני',
  'עטיה חני': 'עטייה חני', 'חני עטיה': 'עטייה חני',
  // תלושי 22.9 (אסתר) — כינוי מול שם מלא, "בר טוב" מול "ברטוב", ושם נעורים
  'בר טוב אלישבע': 'אלישבע ברטוב',
  'וישדסקי דינה': 'דינה וישצקי',
  'צובאלי רות': 'רותי צובלי',
  'בירנהאק חסיה': 'חסי בירנהאק',
  'ביסטריצקי חוה': 'חוי ביסטריצקי',
  'אולבסקי ציפורה': 'ציפי אולבסקי',
  'אקסלרוט ריבקה מיטל': 'רבקי אקסלרוד',
  'זיגמן שולמית': 'שולמית פרידקיס זיגמן', 'שולמית פרידקיס )זיגמן(': 'שולמית פרידקיס זיגמן',
  'בקרמן מנוחה רחל': 'רחלי בקרמן',
  'שניר שטרנע שרה': 'שטערנא שנור',
  // גני תקוה: שתי סגל — 10 שעות (2,882) היא מוסיא, 21 שעות (10,276) היא מושקא
  'סגל חיה מושקא': 'מושקא סגל',
  'סגל מושקא': 'מוסיא סגל',
  // מאומת מטופס 101 שהעובדות מילאו (שם נעורים / שם מלא)
  'הנדל שטרנא שרה': 'שרה אקסלרוד',        // אור עקיבא — לפני הנישואין הנדל
  'בר טוב דבורה לאה': 'יעקובי דבורה לאה', // רמת ישי — "יעקבי (בר-טוב)"
  'שכטר חיה': 'מינדי שכטר',               // גני תקוה — "חיה מינדה שכטר"
  // גני תקוה — שמות נעורים, אישור שרה 22.9 (צעירות שנישאו; הברוטו תואם את המשרה)
  'ליפש מלכה אסתר': 'מלכי ויינטרוב',
  'גולדברג חיה': 'חיה וגנר',
};
...
export function matchRows(rows, teachers, { schoolId = null } = {}) {
  ...
  for (const r of rows) {
    ...
    if (!t) {
      ...
      // חוץ ממצב שבו שתיהן אותה עובדת (הוראה + צהרון, אותו שם ובית ספר):
      // אז שורת ההוראה, שממוינת ראשונה ("חסי בירנהאק", תלושי 22.9).
      const samePerson = (a, b) => a && b && a.schoolId === b.schoolId
        && (String(a.tzId || '') && String(a.tzId) === String(b.tzId) || String(a.name).trim() === String(b.name).trim());
      if (scored.length && (scored.length === 1 || scored[0].s > scored[1].s
          || (scored[0].s === scored[1].s && samePerson(scored[0].x, scored[1].x) && !(scored[2] && scored[2].s === scored[0].s && !samePerson(scored[0].x, scored[2].x))))) {
