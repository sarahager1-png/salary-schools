...
try {
  ...
  */
  ...
  check('שורת סיכום קיימת', t.foot[0] === 'סה"כ' || t.foot[0] === 'סה״כ', t.foot.join(' | '));
  ...
  check('כותרות האקסל זהות לטבלה', JSON.stringify(aoa[0]) === JSON.stringify(t.heads), (aoa[0] || []).join(' | '));
  const xr = aoa.find(x => x[0] === S_PAY);
  check('שורת הסניף באקסל', !!xr, xr ? xr.join(' | ') : 'לא נמצאה');
  check('המספרים באקסל הם מספרים (ניתן לסכם)', !!xr && xr.slice(1).every(v => typeof v === 'number'),
    xr ? xr.slice(1).map(v => typeof v).join(',') : '');
  check('שורת סה"כ באקסל', aoa[aoa.length - 1][0] === 'סה"כ', String(aoa[aoa.length - 1][0]));
